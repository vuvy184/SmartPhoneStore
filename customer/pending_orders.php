            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <?php
                        ...
                        if(isset($_SESSION['customer_id'])){
                            ...
                            $select_customer_orders = "select * from customer_orders where customer_id = '$customer_id' and (status='Đang chờ' or status LIKE 'Không được xác nhận%') ORDER BY order_id DESC";
                            ...
                            while($row_customer_orders = mysqli_fetch_array($run_customer_orders)){
                                ...
                                echo "' style='width: 15%'>$status</td>
                                    <td><a href='order_detail.php?order_id=$order_id' class='btn btn-info text-white'>Xem chi tiết</a></td>
                                    <td class=''>
                                        <form action='' method='post'>
                                            <input type='hidden' name='order_id' value='$order_id'>
                                            <button onclick='cancel_order($order_id)' id='cancel_order$order_id' class='btn btn-white text-danger' style='border: none; padding: 0'><i class='fas fa-times me-1'></i>Hủy</button>
                                        </form>
                                    </td>
                ...
                <script>
                    function cancel_order(id){
                        ...
                        if(result==true){
                            document.getElementById("cancel_order"+id).name = 'cancel_order';
                        }
                    }
                </script>
<?php
    if(isset($_POST['cancel_order'])){
        ...
        $update_customer_orders = "update customer_orders set status='Đã hủy' where order_id='$order_id'";
        ...
        if($run_update){
            ...
            ";
            echo "
                <script>window.open('my_orders.php?pending_orders', '_self')</script>
            ";
        }
    }
?>