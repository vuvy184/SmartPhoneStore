         ...
         <table class="table">
                    ...
                    <?php
                        ...
                        if(isset($_SESSION['customer_id'])){
                            ...
                            $select_customer_orders = "select * from customer_orders where customer_id = '$customer_id' and status='Đã hủy' ORDER BY order_id DESC";
...
<?php
if(isset($_SESSION['customer_id'])&&isset($_POST['buy_again'])){
    $customer_id = $_SESSION['customer_id'];
    ...
    while($row_customer_order_products = mysqli_fetch_array($run_customer_order_products)){
        ...
        $color = $row_customer_order_products['color'];
        $quantity = $row_customer_order_products['quantity'];
        $insert_cart = "insert cart (customer_id, product_id, color, quantity) 
        values ('$customer_id', '$product_id', '$color', '$quantity')";
        $run_insert_cart = mysqli_query($conn, $insert_cart);
        $delete_customer_order_products = "delete from customer_order_products where order_id = '$order_id'";
        $run_delete_customer_order_products = mysqli_query($conn, $delete_customer_order_products);
    }
    $delete_customer_orders = "delete from customer_orders where order_id='$order_id' and status='Đã hủy'";
    $run_customer_orders = mysqli_query($conn, $delete_customer_orders);
